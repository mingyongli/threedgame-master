...
import com.ws3dm.app.activity.GuessNewsListActivity;
 ...
 **/
public class NewsMultiAdapter extends MultiItemRecyclerAdapter<NewsBean> {
    ...
    public NewsMultiAdapter(Context ctx, MultiItemTypeSupport<NewsBean> support) {
        super(ctx, support);
    }
...
//                        Glide.with(mContext).load(item.getTypelist().get(position).getLitpic()), holder.getView(R.id.img_bg);
                       ...
                       // String date=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(item.getPubdate_at()*1000);
                        holder.setText(R.id.time, TimeUtil.getFormatTimeSimple(item.getPubdate_at()));
                        // } else {
