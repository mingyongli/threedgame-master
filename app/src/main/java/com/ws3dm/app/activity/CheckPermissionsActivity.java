...
public abstract class CheckPermissionsActivity extends BaseActivity implements ActivityCompat.OnRequestPermissionsResultCallback {
     ...
     * 需要进行检测的权限数组
     */
    protected String[] needPermissions = {Manifest.permission.WRITE_EXTERNAL_STORAGE, Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.READ_PHONE_STATE};
    /**
     * 判断是否需要检测，防止不停的弹框
     */
    ...
    private void checkPermissions(String... permissions) {
        ...
        } else if (null != needRequestPermissonList && needRequestPermissonList.size() == 0 && !hasCallGetAllPermissions) {
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] paramArrayOfInt) {
        if (requestCode == PERMISSON_REQUESTCODE) {
            if (!verifyPermissions(paramArrayOfInt)) {
                ...
                isNeedCheck = false;
            } else if (!hasCallGetAllPermissions) {
                hasGetAllPermissions();
                hasCallGetAllPermissions = true;
            }
        }
