...
/**
 * Describution : 类似viewpager，左右两边显示小图，从后面出来
 * 
 * Author : DKjuan
 * 
 ...
 *
 ...
 int position = coverFlowView.getTopViewPosition();//获取最上面Item的position
 CoverFlowAdapter.Holder holder = (CoverFlowAdapter.Holder) coverFlowView.getTopView().getTag();//获取最上面Item的View
 break;
 
 *示例 attrs.xml添加
 *     
 <declare-styleable name="ImageCoverFlowView">
 <attr name="visibleImage" format="integer"/>
 <attr name="reflectionHeight" format="fraction"/>
 <attr name="reflectionGap" format="dimension"/>
 <attr name="reflectionShaderEnable" format="boolean"/>
 <attr name="topImageClickEnable" format="boolean"/>
 <attr name="coverflowGravity" format="enum">
 <enum name="top" value="0"/>
 <enum name="bottom" value="1"/>
 <enum name="center_vertical" value="2"/>
 </attr>
 <attr name="coverflowLayoutMode" format="enum">
 ...
 属性说明 CoverFlow 的 Gravity：imageCoverFlow:coverflowGravity="center_vertical"
...
public class CoverFlowView extends RelativeLayout {
    ...
    private final int ALPHA_DATUM = 128; // 基础alphaֵ
    ...
    private static final float MAX_SPEED = 3.0f;//临时注释 初始是6.0f
...
//                    }
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (isOnTopView && touchViewItem == getTopView()
                    ...
                    if (!mTouchMoved&&mTopViewClickListener != null) {
                        mTopViewClickListener.onClick(getTopViewPosition(), getTopView());
                    }
                }
                touchViewItem = null;
    ...
    private void sendLongClickAction() {
        ...
        new Handler().postDelayed(longClickRunnable, 600);//原版postDelayed(longClickRunnable, 600);
    }
